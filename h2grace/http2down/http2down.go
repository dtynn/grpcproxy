// a fork version of github.com/facebookgo/httpdown for http2.Server only
...
// HTTP2 defines the configuration for serving a http.Server. Multiple calls to
// Serve or ListenAndServe can be made on the same HTTP2 instance. The default
...
// net.Listener.
func (h HTTP2) Serve(l net.Listener, s *http2.Server, opts *http2.ServeConnOpts) Server {
...
type server struct {
	...
	server        *http2.Server
	serverConnOpt *http2.ServeConnOpts
...
func (s *server) Stop() error {
	s.stopOnce.Do(func() {
		...
		s.serverConnOpt.BaseConfig.SetKeepAlivesEnabled(false)
...
func Serve(lis net.Listener, s *http2.Server, opts *http2.ServeConnOpts, hd *HTTP2) error {
	if hd == nil {
		hd = &HTTP2{}
	}

	hs := hd.Serve(lis, s, opts)
